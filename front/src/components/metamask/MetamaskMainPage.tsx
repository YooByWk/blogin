...
const MetamaskMainPage = () => {
  ...
  const [chainId, setChainId] = useState<string | number | undefined>(undefined);
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: "row", gap: "10%" }}>
        <Card style={cardStyle} bg='dark' onClick={connectWallet}>
          ...
          {balance === undefined ? (
            <Button onClick={() => signer && getBalance(signer)}> 잔고 가져오기</Button>
          ) : (
            <Card.Title>잔고: {ethers.formatEther(balance)}ETH</Card.Title>
          )}
        </Card>
        <Card style={cardStyle} bg='dark'>{provider && signer && <VersionBtn provider={provider} signer={signer} />}</Card>
        <Card style={cardStyle} bg='dark'>{provider && signer && <AuthorBtn provider={provider} signer={signer} />}</Card>
      </div>
    </div>
  );
};
